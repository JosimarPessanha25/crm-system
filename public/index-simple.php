...
class SimpleCRM {
    ...
    private function getContactsWithCompanies() {
        return array_map(function($contact) {
            $company = array_find($this->companies, function($c) use ($contact) {
    ...
    private function getOpportunitiesWithRelations() {
        return array_map(function($opp) {
            $contact = array_find($this->contacts, function($c) use ($opp) {
                return $c['id'] === $opp['contato_id'];
            });
            $company = array_find($this->companies, function($c) use ($opp) {
